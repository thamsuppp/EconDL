...
Original file is located at
    https://colab.research.google.com/drive/1wFU0HgfWCY0yk7hZpeCzUekI0yO1jl7s
...
### Variable Selection Networks (not fully functional)
"""
...
class VARNN(nn.Module):
    def __init__(self, n_features, n_outputs, nodes, x_pos, dropout_rate, input_dropout_rate, cancel_out, vsn, fcn, neurons_weights, device, actv = 'ReLU()', s_pos = None):
        ...
        # New (11/11) - if only want to pass in a subset of X into the nonlinear part S
        if s_pos:
          n_features = len(s_pos)
          self.s_pos = torch.tensor(s_pos, dtype = torch.int64).to(device)
        else:
          self.s_pos = None
        ...
        if len(nodes) == 1:
          # May be wrong (pls confirm)
          self.first = nn.Linear(nodes[0], nodes[0])
          self.last = nn.ModuleList([nn.Linear(nodes[0], neurons_weight) for neurons_weight in neurons_weights])
        else:
          self.first = nn.Linear(nodes[0], nodes[0])
          self.hidden = nn.ModuleList([nn.Linear(nodes[node_id], nodes[node_id+1]) for node_id in range(len(nodes)-1)])
          ...
          self.tvpl = nn.ModuleList(tvpl_all)
          self.output = nn.ModuleList(output_all)
    ...
    def forward(self, S):
      ...
      # First input into nonlinear part (take the s_pos columns if specified)
      if self.s_pos is not None:
        S = torch.index_select(S, 1, self.s_pos)
      ...
      for i in range(self.n_outputs):
        # For intercept
        x_i = torch.clone(x)
        for l in range(len(self.tvpl[i][0])):
          x_i = self.actv(self.tvpl[i][0][l](x_i))
        betas_alt = self.output[i][0](x_i)
        ...
        for j in range(1, sum(self.n_betas) + 1):
          x_b = torch.clone(x)
          for l in range(len(self.tvpl[i][j])):
            x_b = self.actv(self.tvpl[i][j][l](x_b))
          alt = self.output[i][j](x_b)
      ...
      y_hat = torch.unsqueeze(torch.sum(torch.mul(dat, betas[0]), dim = 1), 1)
      
      for i in range(1, self.n_outputs):
        alt = torch.unsqueeze(torch.sum(torch.mul(dat, betas[i]), dim = 1), 1)
        y_hat = torch.hstack([y_hat, alt])
      return y_hat, betas, v
...
def training_loop_new(X_train, Y_train, model, criterion, optimizer, scheduler, train_indices, nn_hyps):
  ...
  for epoch in range(num_epochs):
    ...
    # if train_losses:
    #   epoch_range.set_description("loss: {:.6f}".format(train_losses[-1]))
    ...
    if epoch % 20 == 0:
      print(f'Epoch: {epoch}, Loss: {loss}, L1 input Loss: {l1_input_loss}, L1 input: {l1_input}, L0 input Loss: {l0_input_loss}, L0 input: {l0_input}')
...
# Overall function to instantiate NN and train

def build_VARNN(X, Y, train_indices, nn_hyps, device):

  if nn_hyps['s_pos']:
    n_features = len(nn_hyps['s_pos'])
  else:
  ...
  scheduler = optim.lr_scheduler.MultiplicativeLR(optimizer, lr_lambda = lmda)
  model = model.to(device)
  
  # Training the built VARNN and return the results
  results = training_loop_new(X, Y, model, criterion, optimizer, scheduler, train_indices, nn_hyps)
...
def conduct_bootstrap(X_train, X_test, Y_train, Y_test, nn_hyps, device):
  ...
  if nn_hyps['prior_shift'] == True:
    ...
    #print('Ytrain', np.cov(Y_train.T))
    #print('Ytrain_fitted', np.cov(Y_train_fitted.T))
    Y_train = Y_train - Y_train_fitted
    #print('Ytrain after', np.cov(Y_train.T))
    Y_test = Y_test - Y_test_fitted

    # Params is 7 x 3
    #print('PS Params', ps_model.params)
  ...
  for j in range(num_bootstrap):
    ...
    if opt_bootstrap == 2: # Block bootstrap
      ...
      #print('block_start_ids', block_start_ids)
    ...
    if oob_loss_multiple > oob_loss_multiple_threshold:
      ...
      # preds is 169 x 2
      # betas is a list of 2, each element is 169 x 2 matrix
      in_preds, in_betas, _ = model(X_train[boot, :])
      #print('in_betas', in_betas[0].shape)
      #print('boot', len(boot))
      ...
      if nn_hyps['standardize'] == True:
        ...
        if nn_hyps['vsn'] == False and nn_hyps['fcn'] == False:
            # Store the betas
            for i in range(Y_train.shape[1]):
              betas_in_draws[boot, :, j, i] = in_betas[i].detach().cpu().numpy()
              betas_draws[oob, :, j, i] = oob_betas[i].detach().cpu().numpy()
              betas_draws[oos, :, j, i] = test_betas[i].detach().cpu().numpy()
    
              # Invert scaling for the constant term
              betas_draws[:, 0, j, i] = betas_draws[:, 0, j, i] * scale_output['sigma_y'][i] + scale_output['mu_y'][i]
              betas_in_draws[:, 0, j, i] = betas_in_draws[:, 0, j, i] * scale_output['sigma_y'][i] + scale_output['mu_y'][i]
    
              # For non-constant terms
              for k in range(1, n_betas):
                # Scale the kth beta
                betas_draws[:, k, j, i] = betas_draws[:, k, j, i] * scale_output['sigma_y'][i] / scale_output['sigma_x'][x_pos_flat][k-1]
                # Subtract the (lagged variable's mean multiplied by the beta) from the constant term
                betas_draws[:, 0, j, i] = betas_draws[:, 0, j, i] - betas_draws[:, k, j, i] * scale_output['mu_x'][x_pos_flat][k-1]
    
                betas_in_draws[:, k, j, i] = betas_in_draws[:, k, j, i] * scale_output['sigma_y'][i] / scale_output['sigma_x'][x_pos_flat][k-1]
                # Subtract the (lagged variable's mean multiplied by the beta) from the constant term
                betas_in_draws[:, 0, j, i] = betas_in_draws[:, 0, j, i] - betas_in_draws[:, k, j, i] * scale_output['mu_x'][x_pos_flat][k-1]
...
def process_varnn_data(x_d, nn_hyps, marx = True, test_size = 60, n_time_trends = 0, time_dummies = False, dummy_interval = 12):
  n_var = x_d.shape[1]
